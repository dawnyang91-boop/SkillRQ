...
python3 -m skillrq m7 joint-train \
  "${COMMON_ARGS[@]}" \
  --output-root "runs/m7_joint_reranker/${TARGET}/joint_base" \
  --swanlab-run-name "m7-${TARGET}-joint-base"

python3 -m skillrq m7 joint-train \
  "${COMMON_ARGS[@]}" \
  --enable-shared-encoder \
  --output-root "runs/m7_joint_reranker/${TARGET}/shared_encoder" \
...
python3 -m skillrq m7 joint-train \
  ...
  --output-root "runs/m7_joint_reranker/${TARGET}/soft_code_distribution" \
...
python3 -m skillrq m7 joint-train \
  ...
  --enable-soft-code-distribution \
  --soft-code-weight "${SOFT_CODE_WEIGHT:-0.1}" \
  --output-root "runs/m7_joint_reranker/${TARGET}/shared_encoder_soft_code" \
