...
def prepare_code_path_planning_data(
    ...
    for query in read_jsonl(m4_data_root / "queries.jsonl"):
        predicted_paths = predictions.get(str(query["query_id"])) or _oracle_predicted_paths(query)
...
def predict_code_path_plan(
    ...
    for query in queries:
        ...
        for step_index in range(max_steps):
            ...
            retrieved = _retrieve_for_path(selected, candidates, candidates_per_step)
    ...
    logger.log({"predict/queries": summary["queries"], "predict/avg_steps": summary["avg_steps"], "predict/stop_threshold": stop_threshold}, step=1)
    logger.finish()
    return summary


def _load_m4_predictions(path: Path | None) -> dict[str, list[Mapping[str, Any]]]:
    if not path:
        return {}
    return {str(row["query_id"]): list(row.get("predicted_code_paths") or []) for row in read_jsonl(path)}
...
def _retrieve_for_path(path, candidates, limit: int):
    rows = []
    codes = [str(item) for item in path.get("codes") or []]
    for candidate in candidates:
        ...
        if overlap == 0:
            continue
        score = float(path.get("probability") or path.get("score") or 0.0) * (overlap / max(len(candidate_codes), 1))
        rows.append(
            {
